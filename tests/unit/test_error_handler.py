...
from aws_lambda_powertools.utilities.error_handler.http_response import HttpResponse, DEFAULT_HTTP_ERROR_MESSAGE
...
def test_sqs_destination_build_error_message():
    sqs: SqsDestination = SqsDestination(lambda_context=generate_context(), exception=Exception('I failed'), logger=get_logger(),
                                         trace=TRACE)
...
def test_factory_initialization_sqs():
    ...
    sqs_error_handler = ErrorHandlerFactory().get_handler(handler_type=ErrorDestinationEnum.SQS, logger=get_logger(),
                                                          exc=Exception('I failed'), context=mock_lambda_context, trace=TRACE)
...
def test_factory_initialization_http():
    ...
    http_error_handler = ErrorHandlerFactory().get_handler(handler_type=ErrorDestinationEnum.HTTP_RESPONSE, logger=get_logger(),
                                                           exc=Exception('I failed'), context=generate_context(), trace=TRACE)
...
def test_factory_initialization_assertion():
    ...
    sqs_error_handler = ErrorHandlerFactory().get_handler(handler_type=ErrorDestinationEnum.RAISE_EXCEPTION, logger=get_logger(),
                                                          exc=Exception('I failed'), context=generate_context(), trace=TRACE)
...
def test_sqs_handler_missing_env_var(mocker):
    with pytest.raises(ErrorHandlerException, match=f'missing environment variable for SQS DLQ destination error handler.*'):
...
def test_invalid_definition_custom_handler(mocker):
    with pytest.raises(ErrorHandlerException, match='custom destination must be used in conjunction only with ErrorDestinationEnum.CUSTOM'):
