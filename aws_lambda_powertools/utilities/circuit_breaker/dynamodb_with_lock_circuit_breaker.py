...
class DynamoDBLockCircuitBreaker(BaseCircuitBreaker):
    """
        ...
        This client uses DynamoDBLock instance to protect the read/write operation against the DynamoDB CircuitBreaker.
        ...
        Parameters:
            ...
            lock_client: LockClient
                The lock client which this Circuit breaker will use
        ...
        LOCK_CLIENT = DynamoDBLockClient(
            ...
            owner_name=''.join(random.choice(string.ascii_lowercase) for i in range(10)),
    ...
    def _put_circuit_breaker_item_in_table_with_lock(self, circuit_breaker_obj: CircuitBreakerDynamoDBSchema):
        ...
        try:
            lock = self.lock_client.acquire_lock(partition_key=self._name)
            item = self._put_circuit_breaker_item_in_table(circuit_breaker_obj)
        except DynamoDBLockError as ex:
    ...
    def _update_circuit_breaker_item_in_table_with_lock(self, circuit_breaker_obj: CircuitBreakerDynamoDBSchema):
        ...
        try:
            lock = self.lock_client.acquire_lock(partition_key=self._name)
            item = self._update_circuit_breaker_item_in_table(circuit_breaker_obj)
        except DynamoDBLockError as ex:
...
def circuit(name, failure_threshold=None, recovery_timeout=None, expected_exception=None, fallback_function=None,
    ...
    if callable(failure_threshold):
        ...
        return cls(failure_threshold=failure_threshold, recovery_timeout=recovery_timeout, expected_exception=expected_exception, name=name,
